def process_image_ego_prompt(action, object_name):
    return f"""
        ...
        - Report each keypoint as **normalized coordinates (u, v)** where:
          - u = x / image_width  (0.0 to 1.0, left → right)
          - v = y / image_height (0.0 to 1.0, top → bottom)

        ⛔ Do NOT:
        - Mention pixel coordinates, bounding boxes, labels, or explanations.
        - Output anything other than the coordinate list.

        ✅ Output format (strict JSON array of arrays):
        [
          [u1, v1],
          [u2, v2],
          [u3, v3]
